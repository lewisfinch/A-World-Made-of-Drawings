...
export class Ground extends gfx.Mesh3 {
    ...
    public reshapeGround(stroke2D: Stroke2D, groundStartPoint: gfx.Vector3, groundEndPoint: gfx.Vector3, camera: gfx.Camera): void {
        ...
        const groundDif = gfx.Vector3.subtract(groundEndPoint, groundStartPoint);
        const groundNormal = gfx.Vector3.cross(groundDif, gfx.Vector3.UP);
        groundNormal.normalize();
        const projectionPlane = new gfx.Plane3(groundStartPoint, groundNormal);

        const hillCurve: gfx.Vector3[] = [];
        for (const vertex2D of stroke2D.vertices) {
            ...
            if (intersection) {
                hillCurve.push(intersection);
            }
        }

        for (const vertex of this.vertices) {
            const ray = new gfx.Ray3(vertex, projectionPlane.normal);
            const intersection = ray.intersectsPlane(projectionPlane);

            if (intersection) {
                const d = intersection.distanceTo(vertex);
                const h = this.computeH(intersection, hillCurve, projectionPlane);
                const w = Math.max(0, 1 - Math.pow(d / 5, 2));
                vertex.y = (1 - w) * vertex.y + w * (h + vertex.y)
            }
        }
